#!/usr/bin/env python3
"""
Simple Analysis of StudyChat Cognitive Presence Results
Creates key visualizations and explanations for understanding the results.
"""
...
def create_overview_dashboard(data):
    ...
    bars = axes[1,0].bar(metric_names, means, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'])
    axes[1,0].set_title('CP-Bench Metrics (Mean Values)')
...
def create_detailed_analysis(data):
    ...
    # 1. Stage transitions within threads
    ...
    stage_confidence = data['raw'].groupby('raw_stage')['raw_confidence'].agg(['mean', 'std', 'count'])
    ...
    # 4. Role × Stage Distribution
    pivot_data = data['role_stage'].pivot(index='speaker_type', columns='final_stage', values='pct').fillna(0)
    im2 = axes[1,1].imshow(pivot_data.values, cmap='Blues', aspect='auto')
    axes[1,1].set_title('Role × Stage Distribution (%)')
    axes[1,1].set_xlabel('Cognitive Stage')
    axes[1,1].set_ylabel('Speaker Type')
    ...
    for i in range(len(pivot_data.index)):
        for j in range(len(pivot_data.columns)):
            text = axes[1,1].text(j, i, f'{pivot_data.iloc[i, j]:.1f}',
                                 ha="center", va="center", color="black" if pivot_data.iloc[i, j] < 50 else "white")
    
    plt.colorbar(im2, ax=axes[1,1], label='Percentage')
